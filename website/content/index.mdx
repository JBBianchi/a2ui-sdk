...
**⚠️ Important:** V0.9 is a draft version based on the A2UI specification as of 2026-03-11. The v0.9 protocol has changed significantly recently and may continue to evolve. **We recommend using the stable v0.8 until v0.9 reaches alpha or beta status.**
...
// Create a function registry for local function execution (validation, formatting, etc.)
const functionRegistry = createStandardFunctionRegistry()
...
- **Actions** are now a discriminated union: `EventAction` (dispatched to server) or `FunctionCallAction` (executed locally via function registry)
- **Validation** uses condition-based `CheckRule` with `{ condition: DynamicBoolean, message: string }`
- **CreateSurface** requires a `root` field specifying the root component ID
- **New providers**: `ThemeProvider`, `ErrorProvider`, `FunctionRegistryProvider` (all included in `A2UIRenderer`)
- **Accessibility**: Components support `accessibility: { label, description }` attributes
...
// Custom button component using v0.9 action types
function CustomButtonComponent({ surfaceId, componentId, child, action }) {
  const dispatchAction = useDispatchAction()
